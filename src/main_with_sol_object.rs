...
#[derive(Debug)]
struct Solution(HashMap<Node, i64>);
...
impl Solution {
    ...
    fn update(&mut self, node: &Node, val: i64) {
        self.0.insert(node.clone(), val);
    }

    fn add_var_if_missing(&mut self, node: &Node) {
...
impl DCS{
    ...
    fn add_constraint_helper(&mut self, x: usize, y: usize, a: i64, sol: &mut Solution) -> bool {
        self.add_constraint(&Node::Variable(x), &Node::Variable(y), a, sol)
    }
    fn add_constraint(&mut self, x: &Node, y: &Node, a: i64, sol: &mut Solution) -> bool{
        // x - y  <= a
